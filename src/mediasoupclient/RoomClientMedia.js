...
export class RoomClientMedia {
  //localMediaEl, remoteVideoEl, remoteAudioEl,
  constructor(mediasoupClient, socket, room_id, name, userData, successCallback) {
    ...
    this.localMediaEl = //document.createElement('video');
    this.remoteVideoEl = document.createElement('video'); //remoteVideoEl
    this.remoteAudioEl = document.createElement('audio'); //remoteAudioEl
    ...
    this.createRoom(room_id).then(
      async function () {
        await this.join(name, room_id, userData)
        this.initSockets()
        this._isOpen = true
        successCallback()
      }.bind(this)
  ...
  async join(name, room_id, userData) {
    this.socket
      .request('join', {
        room_id,
        name,
      ...
      .then(
        async function (e) {
          console.log('Joined to room', JSON.parse(JSON.stringify(e.id)), JSON.parse(JSON.stringify(e.peers)))
  ...
  async produce(type, deviceId = null, calleeId = null) {

    //console.log(type, deviceId, calleeId)
    ...
    try {
      ...
      let elem
      if (!audio) {
        elem = this.localMediaEl //document.createElement('video')
  ...
  async consume(producer_id) {
    ...
    this.getConsumeStream(producer_id).then(
      function ({ consumer, stream, kind }) {
        ...
        let elem
        if (kind === 'video') {
          elem = this.remoteVideoEl//document.createElement('video')
          elem.srcObject = stream
          elem.id = consumer.id
          elem.playsinline = false
          elem.autoplay = true
          elem.className = 'vid'
          this.remoteVideoEl = elem;
          //this.remoteVideoEl.appendChild(elem)
          this.handleFS(elem.id)
        } else {
          elem = this.remoteAudioEl //document.createElement('audio')
          elem.srcObject = stream
          elem.id = consumer.id
          elem.playsinline = false
          elem.autoplay = true
  ...
  onaddstream(streams, videoCallId) {
    let childVid = document.createElement('video');//this.renderer.createElement('video');
  ...
  removeElementFromDiv(videoCallId) {
    let divElement = document.getElementById(videoCallId)
    Array.from(divElement.children).forEach(child => {
      console.log('children.length=' + divElement.children.length);
      document.removeChild(child);
      //this.renderer.removeChild(divElement, child);
    });
  }
